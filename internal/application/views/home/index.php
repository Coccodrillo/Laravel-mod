<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<title>Dobrodošli na potni.si!</title>

	<link href="http://fonts.googleapis.com/css?family=Quattrocento&amp;v1" rel="stylesheet" type="text/css" media="all" />
	<link href="http://fonts.googleapis.com/css?family=Ubuntu&amp;v1" rel="stylesheet" type="text/css" media="all" />
	<link href='http://fonts.googleapis.com/css?family=Lobster+Two' rel='stylesheet' type='text/css'>

	<style type="text/css">
		body {
			background-color: #eee;
			color: #6d6d6d;
			font-family: 'Ubuntu';
			font-size: 15px;
		}

		a {
			color: #7089b3;
			font-weight: bold;
			text-decoration: none;
		}

		h1.laravel {
			font-family: 'Lobster Two', Helvetica, serif;
			font-size: 60px;
			margin: 0 0 15px -10px;
			padding: 0;
			text-shadow: -1px 1px 1px #fff;
		}

		h2 {
			font-family: 'Lobster Two', serif;
			font-size: 30px;
			margin: 30px 0 0 0;
			padding: 0;
			text-shadow: -1px 1px 1px #fff;
		}

		p {
			margin: 10px 0 0 0;
			line-height: 25px;
		}

		#header {
			margin: 0 auto;
			margin-bottom: 15px;
			margin-top: 20px;
			width: 80%;
		}

		.wrapper {
			background-color: #fff;
			border-radius: 10px;
			margin: 0 auto;
			padding: 10px;
			width: 80%;
		}

		.wrapper h2:first-of-type {
			margin-top: 0;
		}
	</style>
</head>
<body>
	<div id="header">
		<h1 class="laravel">Potni.si</h1>
	</div>

	<div class="wrapper">
		<h2>Tole je landing page za nastajajočo aplikacijo potni.si</h2>

		<p>Beta dostop bo omogočen v kratkem.</p>
	</div>
</body>
</html>

